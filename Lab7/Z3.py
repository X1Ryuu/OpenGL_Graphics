...
def startup():
    ...
    glVertexAttribPointer(0, 3, GL_FLOAT, GL_TRUE, 6 * ctypes.sizeof(ctypes.c_float), ctypes.c_void_p(0))  # Pozycja
    glEnableVertexAttribArray(0)

    glVertexAttribPointer(1, 4, GL_FLOAT, GL_FALSE, 6 * ctypes.sizeof(ctypes.c_float),
                          ctypes.c_void_p(3 * ctypes.sizeof(ctypes.c_float)))  # Kolor
...
def render(time):
    ...
    V_matrix = glm.lookAt(
        glm.vec3(0.0, 0.0, 5.0),  # Oddalenie kamery, aby objąć większy obszar
    ...
    for i in range(-5, 5):
        for j in range(-5, 5):
            M_matrix = glm.translate(glm.mat4(1.0), glm.vec3(i * 0.6, j * 0.6, 0.0))
