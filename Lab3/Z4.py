...
#     right = button == GLFW_MOUSE_BUTTON_RIGHT and action == GLFW_PRESS
#     if left:
...
def render(time):
    ...
    if e_button_state:
        ...
        print('viewer: ', viewer[0], viewer[1], viewer[2])
        if left_mouse_button_pressed:
            theta += delta_x * pix2angle
            phi += delta_y * piy2angle
        glRotatef(theta, 0.0, 1.0, 0.0)
    ...
    # Obliczenia pozycji kamery na podstawie theta, phi i R
    axes()
    example_object()
    glFlush()
